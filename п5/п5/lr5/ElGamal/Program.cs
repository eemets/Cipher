...
namespace ElGamal
{
    internal class Program
    {
        ...
        private static int Power(int a, int b, int n){ // a^b mod n - возведение a в степень b по модулю n
            var tmp = a;
            var sum = tmp;
            for(var i = 1; i < b; i++){
                for(var j = 1; j < a; j++){
        ...
        private static int Mul(int a, int b, int n){ // a*b mod n - умножение a на b по модулю n
            var sum = 0;
            for(var i = 0; i < b; i++){
                sum += a;
                if(sum >= n)
